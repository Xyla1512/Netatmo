...
### Added
- **Retention of raw readings, switchable.** The settings page promised "all data is stored permanently" while the dashboard sidebar showed a "Data Retention: 365" that nothing applied — the value had no field and no effect, and the purge button was the only way to delete anything. Retention is a real thing now, in a section of its own on the settings page and off by default: switch it on, give it a number of days (365 by default, never fewer than 30), and once a night, after the daily summary, the plugin deletes raw readings older than that and notes the run on the settings page and in the cron log. Only the raw readings go — the ten-minute values behind the live dashboard, `[naws_table]`, `[naws_chart]`, the wind rose and the REST readings endpoint. The daily table is never touched, so history, heatmap, records and climate indices keep their full range. With the retention on, the wind rose's "everything recorded" period calls itself "everything kept (last 365 days)". An update changes nothing: the switch is off until you turn it on. The purge button stays for the manual case and takes the configured days as its default.

### Fixed
- **The wind cards of the live dashboard ignored every colour setting.** `[naws_live]` draws its compass, the pointer and the half-circle gauge for wind and gusts in the script, and until now with literal colours: the pointer was always `#c0392b`, the compass face `#f4fafa`, the wind arc `#427272`, the gust arc `#7aa0a0`. The "Compass Needle (Wind Rose)" colour on the Base Theme tab went into the page as a variable that nothing read any more — the class that used it belonged to a compass no template has rendered for a long time — while that tab's preview painted the compass in the theme colours, promising what the frontend never did. For wind and gusts there was no field at all. The two cards now take their colours from a tab of their own, Appearance › Live dashboard: wind, with six fields: background (shared by the compass face and the empty part of the gauge), compass rose, compass pointer (the old "Compass Needle", same key, so a saved colour survives the move), gauge wind, gauge gusts, gauge peak gust of the day. Rings, letters and ticks follow the base theme. The defaults are exactly the colours the script used to hard-code, so an installation that never touched them looks as before — with three small exceptions: the pointer's default is now that red (`#c0392b`) rather than the `#ef4444` the old preview showed, the gauge's wind needle takes the wind colour like its arc instead of the dark text colour, and the empty part of the gauge takes the compass background instead of the border colour, so the two cards match. The tab's preview is the real compass and gauge in the chosen colours, and the four theme colours they borrow follow the Base Theme pickers live.
- **The wind rose said "no wind readings" for a period that had readings — all of them calm.** A period with nothing but calm (every reading below 1 km/h) has no direction to draw, and the panel showed "No wind readings in this period" right under a meta line counting the readings. It now says what happened: "Calm the whole time: 20 readings, all below 1 km/h." Readings without a valid direction still get the old sentence.
- **The translators comments never reached the catalogues.** Fifty-six `/* translators: … */` comments in the code explain what a `%s` or `%d` stands for, and not one of them was in the shipped `.pot` or the `.po` files: the extractor looked for them inside the call's parentheses, where WordPress convention never puts them. It looks before the call now — directly before it, in the enclosing `sprintf`, or on the line above — and stops at the start of the statement or list item, so no string inherits its neighbour's comment.

## [2.0.1]

### Fixed
- **The 24-hour rain in the dashboard's rain card was far too low.** The card sums the plugin's own `Rain` readings over the last 24 hours, because Netatmo's `sum_rain_24` resets at midnight and the card promises a rolling day. But the rain gauge reports every five minutes, each report carrying the rain of those five minutes, and `getstationsdata` shows only the newest report — so a fetch every ten minutes stored one report in two, and the card showed 1.6 mm of a day on which 3.9 mm fell (a fetch every 30 or 60 minutes lost even more). After every fetch the plugin now asks `getmeasure` for the five-minute reports since the last one it has, and stores them under their own timestamps; the first fetch after the update closes the last 24 hours in one call. A dry hour needs no extra call: the gauge's "rain in the last hour" is zero, so the skipped reports were zeros too. The daily, monthly and yearly sums were never affected — they come from Netatmo's own daily counter. The rain rule of the e-mail notifications, which uses the same rolling sum, is corrected along with the card.
...
- **The heatmap builds up when it comes into view, not when the page loads.** The wave of tiles ran at page load, and on any page where `[naws_heatmap]` sits below the fold it was over before anyone scrolled to it. The tiles now stay hidden until the top edge of the map reaches the middle of the screen — or until the page is scrolled to its end, for a map near the bottom of a short page that never gets that far; then the wave runs once, as before. Visitors who ask for reduced motion, and print, get the map at once; without JavaScript nothing is hidden in the first place. Switching the year before the map has appeared ends the wait.

## [2.0.0]

### Added
- **E-mail notifications.** After every fetch the plugin checks up to thirteen rules and mails every change of state — once when it begins and, for every rule but rain, once when it is over; all changes of one fetch go into one mail. Seven rules watch the station: battery of a module below a percentage, weak radio link of a module, poor Wi-Fi of the base station, base station not reporting to Netatmo, module not reporting to the base station, three failed fetches in a row, expired credentials. Six watch the weather and the indoor air: frost, heat, a gust above a threshold, rain of the last 24 hours above a threshold, rain starting (rain in the last hour, one mail per rain), CO₂ of the base station or an indoor module above a threshold. Every rule has a switch, most a threshold. Radio and Wi-Fi must stay poor for 30 minutes before they speak; the two silence rules speak at once and give the all-clear only after 30 quiet-free minutes; frost and gust wait an hour below the threshold before the all-clear; rain sends no all-clear, one mail per rain event. All rules ship switched off. A master switch pauses every mail without touching the rules. The new page XTX Netatmo → Notifications holds a master switch, the recipients, the rules, a test-mail button and the last fifty mails, six visible and the rest in a scroll box.
