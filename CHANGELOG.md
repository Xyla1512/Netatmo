...
- **The heatmap builds up when it comes into view, not when the page loads.** The wave of tiles ran at page load, and on any page where `[naws_heatmap]` sits below the fold it was over before anyone scrolled to it. The tiles now stay hidden until the top edge of the map reaches the middle of the screen, or until the whole map is in view on a short page that never gets it that far; then the wave runs once, as before. Browsers without IntersectionObserver, visitors who ask for reduced motion, and print get the map at once; without JavaScript nothing is hidden in the first place. Switching the year before the map has appeared ends the wait.
